...
public class Painter extends JPanel {
	...
	// this toggles the saving of datpoints on in the universe class to output
	// the trails
	boolean trails = true;
	...
	public void paintComponent(Graphics g) {
		...
		for (Entity e : bodies) {
			// if the mass is greater than a certain threshold scale it down and
			// make it red
			if (e.mass > 300) {
				g.setColor(Color.RED);
				drawCenteredCircle(g, 640 + ((int) e.x),
						360 + ((int) e.y * -1),
						(int) (2 + (int) e.mass) / (100));
				if (trails) {
					// if the trails are enabled output the past history of
					// points saved by each body
					for (int k = 0; k < e.pastLoco.size() - 1; k++) {
						g.drawLine(640 + e.pastLoco.get(k).x,
								360 + (e.pastLoco.get(k).y * -1),
								640 + e.pastLoco.get(k + 1).x,
								360 + (e.pastLoco.get(k + 1).y * -1));
					}
				}
				g.setColor(Color.BLUE);
			} else if (e.mass > 100) {
				...
				drawCenteredCircle(g, 640 + ((int) e.x),
						360 + ((int) e.y * -1), (int) (2 + (int) e.mass) / (10));
				if (trails) {
					// if the trails are enabled output the past history of
					// points saved by each body
					for (int k = 0; k < e.pastLoco.size() - 1; k++) {
